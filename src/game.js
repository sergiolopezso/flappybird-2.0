const game = {
  ...
  setDimensions() {
    ...
    document
      ...
      .setAttribute("width", this.canvasSize.w);
    document
      .querySelector("#myCanvas")
      .setAttribute("height", this.canvasSize.h);
  },
  setEventListeners() {
  ...
  clearPipelines() {
    this.pipelines = this.pipelines.filter(
      (pipe) => pipe.pipelineSpecs.pos.x >= 0 - pipe.pipelineSpecs.size.width
    );
  },
  ...
  clearPowerups() {
    this.powerups = this.powerups.filter(
      (powerup) =>
        powerup.powerupSpecs.pos.x >= 0 - powerup.powerupSpecs.size.width
    );
  },
  ...
  takePowerup() {
    return this.powerups.some((powerup, i) => {
      if (
        // condicion eje x
        this.bird.birdSpecs.pos.x + this.bird.birdSpecs.size.w >=
          powerup.powerupSpecs.pos.x &&
        this.bird.birdSpecs.pos.x <=
          powerup.powerupSpecs.pos.x + powerup.powerupSpecs.size.width &&
        // condicion eje y
        this.bird.birdSpecs.pos.y + this.bird.birdSpecs.size.h >=
          powerup.powerupSpecs.pos.y &&
        this.bird.birdSpecs.pos.y <=
          powerup.powerupSpecs.pos.y + powerup.powerupSpecs.size.height
      ) {
  ...
  clearPowerdowns() {
    this.powerdowns = this.powerdowns.filter(
      (powerdown) =>
        powerdown.powerdownSpecs.pos.x >=
        0 - powerdown.powerdownSpecs.size.width
    );
  },
  ...
  takePowerdown() {
    return this.powerdowns.some((powerdown, i) => {
      if (
        ...
        this.bird.birdSpecs.pos.x <=
          powerdown.powerdownSpecs.pos.x +
            powerdown.powerdownSpecs.size.width &&
        // condicion eje y
        this.bird.birdSpecs.pos.y + this.bird.birdSpecs.size.h >=
          powerdown.powerdownSpecs.pos.y &&
        this.bird.birdSpecs.pos.y <=
          powerdown.powerdownSpecs.pos.y + powerdown.powerdownSpecs.size.height
      ) {
        this.powerdowns = this.powerdowns.filter(
          (p) => p !== this.powerdowns[i]
        );
        return true;
      }
    });
  ...
  counting() {
    this.pipelines.forEach((pipeline) => {
      ...
      if (
        this.bird.birdSpecs.pos.x >
          pipeline.pipelineSpecs.pos.x + pipeline.pipelineSpecs.size.width &&
        this.bird.birdSpecs.pos.x <
          pipeline.pipelineSpecs.pos.x + pipeline.pipelineSpecs.size.width + 10
      ) {
  ...
  isCollision() {
    ...
    return this.pipelines.some((pipeline) => {
      return (
        // condicion eje x
        this.bird.birdSpecs.pos.x + this.bird.birdSpecs.size.w >=
          pipeline.pipelineSpecs.pos.x &&
        this.bird.birdSpecs.pos.x <=
          pipeline.pipelineSpecs.pos.x + pipeline.pipelineSpecs.size.width &&
        // condicion eje y
        (this.bird.birdSpecs.pos.y <= pipeline.pipelineSpecs.size.height ||
          this.bird.birdSpecs.pos.y + this.bird.birdSpecs.size.h >=
            pipeline.pipelineSpecs.size.height +
              pipeline.pipelineSpecs.spaceBetween)
      );
    });
  },
